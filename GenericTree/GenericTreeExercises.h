...
static void treeFactory(GenericTree<int>& tree) {
  ...
  auto child111 = child11->addChild(42);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  bfs.push(rootNodePtr);
  
  while(!bfs.empty())
  { 
     auto thisNode = bfs.front();
     results.push_back(thisNode->data);
     bfs.pop();
     for(int i=0;i<thisNode->childrenPtrs.size();i++)
     {
      if(thisNode->childrenPtrs[i] != nullptr)
        {
          bfs.push(thisNode->childrenPtrs[i]);
        }  
     }
  }
  return results;
